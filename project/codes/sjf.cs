    ...
    public static float nonpreemptive(process[] p, int n,Scheduling x)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            if (p[i].arrival_time < count)  //lw fih 7aga gt wna lsa m5lstsh ..7shoof as3'r 7aga gt wa5liha tbtdy b3d mana a5ls
            {
                ...
                smallest = process.smallest(p, i, 1, n, count);
            ...
            {   if (i!=n-1)
                if ((p[i].arrival_time == p[i + 1].arrival_time) && (p[i + 1].burst_time < p[i].burst_time))
                {
                    temp.copy(p[i]);
                    p[i].copy(p[i + 1]);
                    p[i + 1].copy(temp);
      ...
      //  float average_turnaaround_time = 0;
        ...
        while (arrival.Count() != 0 || (waiting.Count() != 0||count ==0))
        {
            ...
            if (arrival.Count() != 0 && flag != 0)
                ...
                while (count > temporary8 && flag != 0&& arrival.Count() != 0)  //lw gh w2t awl wa7ed
                {
                    if (arrival.Count() != 0)
                    {
                        ...
                        temporary14.priority = temporary14.burst_time;
              ...
              if (arrival.Count() != 0 && flag != 0)
                ...
                while (count == temporary6 && flag != 0&& arrival.Count() != 0)  //lw gh w2t awl wa7ed
                {
                    if (arrival.Count() != 0)
                    {
                        ...
                        smallest.priority = smallest.burst_time;
            ...
            if (count == temporary7.arrival_time)
            {
                ...
                temporary10 = arrival.Peek().burst_time;
                temporary12 = arrival.Peek().arrival_time;
                if ((temporary10 < temporary2.burst_time)&&temporary7.arrival_time==temporary12)
                {
                    temporary11.copy(arrival.Dequeue());
                    arrival.Enqueue(temporary2);
                    exec.copy(temporary11);
                    exec.priority = exec.burst_time;
                ...
                else
                {
                    ...
                    exec.priority = exec.burst_time;
                    temporary2.priority = temporary2.burst_time;
            ...
            temporary4 = waiting.Peek().burst_time;
            if (waiting.Count() != 0 && flag != 0)
                temporary9 = waiting.Peek().arrival_time;

            if (exec.burst_time>temporary4&&exec.arrival_time!=temporary9&&waiting.Count()!=0&&flag!=0 && exec.finished != true) //l2it wa7ed as3'rs
              {
                  exec.end = count;
                  ...
                  temporary.priority = exec.priority;
                  temporary.finished = false;
                  if ((exec.burst_time + exec.begin != count))
                  {
                    temporary.priority = temporary.burst_time;
            ...
            if ( arrival.Count() == 0 && waiting.Count() == 0&&flag!=0)
                ...
                Console.WriteLine(exec.name +" finished "+ finish);
                exec.finished = true;
                end = finish;
